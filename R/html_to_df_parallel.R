library(doParallel);
n_cores = detectCores();
cl = makeCluster(4, outfile = "C:/Users/nikhi/Downloads/log.txt"); # outfile = "C:/Users/nikhi/Downloads/log.txt"
registerDoParallel(cl);
# stopCluster(cl)

# For linux use library(doMC); and registerDoMC(24);
...
system.time( # 1:len(cusips)
  all_failed <- foreach(z = 1:len(cusips), .combine = rbind) %dopar% {
    process_file(z)
  }
)

# 1:25 --> 123, 443 i.e. a gain of 3.6 times with parallel computation!


# keep a copy for future diagnosis
failed_files_forever = rbind(failed_files_forever, all_failed);
saveRDS(failed_files_forever, forever_failed_file, compress = F);

# now delete all failed html files
file.remove(all_failed$file);
# need to remove the above html files from history file as well
dt = fread(paste0(root_dir, "html_history.csv"));
del = dt[file %in% str_replace_all(all_failed$file, "/", "\\\\"), .(file)];
setkey(dt, file);
dt = dt[!del];
